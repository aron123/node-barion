...
const { setFieldsOptional, getRequiredFields } = require('./schema');
...
function sanitizeThenValidate (schema, object, justSanitize) {
    let fieldsToSetOptional = [];

    if (justSanitize) { // TODO: put sanitize-only mode to other function
        const requiredFields = getRequiredFields(schema);
        fieldsToSetOptional = difference(requiredFields, immutableFields);
    }

    const validationSchema = justSanitize ? setFieldsOptional(schema, fieldsToSetOptional) : schema;
