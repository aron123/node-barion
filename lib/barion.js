 ...
 */
function getBaseUrl () {
    return baseUrls[defaults.Environment];
}
...
function Barion (options) {
    ...
    if (defaults.Environment != 'prod' && defaults.Environment != 'test') {
        throw new Error(`Environment property must be 'test' or 'prod' (got '${defaults.Environment}' instead)`);
    }
}
